...
import numpy as np
import torch
from model.utilis.data_utilis import *
import joblib
from model.OuterFrame.BiEncoder import *
from model.OuterFrame.MLP import *
from model.OuterFrame.Decoder import *
from model.InternalFrame.TF_model import *
...
drug_file = resource_path("data", "DB_smiles_129vec.csv") # If drugs ID is prefixed with "BRD" please replace with "BRD_smiles_129vec.csv"
gene_file = resource_path("data", "geneid2vec128_978.csv")



def Predicting_process(drug_file, gene_file, **kwargs):
    CE_model = TF(drug_input_dim=kwargs['drug_input_dim'], gene_input_dim=kwargs['gene_input_dim'], drug_gene_embed_dim=kwargs['drug_gene_emb_dim'], 
                device=kwargs['device'], hid_dim=kwargs['hid_dim'], num_gene=kwargs['gene_num'], TFlatent_dim=kwargs['CE_latent_dim'],drop=kwargs['CE_drop'],TF_output_dim=kwargs['CE_output_dim'],
    ...
    target_dsnae = BiEncoder(private_encoder=CE_model,share_encoder=share_encoder,decoder=share_decoder,**kwargs)
    ...
    target_dsnae_path = resource_path("results", "trainedModel", "target_be.pt")
    map_location = kwargs['device']
    target_dsnae.load_state_dict(torch.load(target_dsnae_path, map_location=map_location))
    predictor_path = resource_path("results", "trainedModel", "predictor.pt")
    predictor.load_state_dict(torch.load(predictor_path, map_location=map_location))
    result_path = resource_path("results", "trainedModel")
    batch_size = 128
    ...
    data_path = resource_path("data", "TestData.csv") # the path of the data to be predicted
    ...
    with torch.no_grad():
        ...
        output_dir = os.path.join(os.getcwd(), "results", "testOutput")
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, "testPrediction.csv")
        pred_data.to_csv(output_file, header=gene_vec.index.tolist(), index=False) # the path of the prediction result
        if kwargs['device'].type == "cuda":
            torch.cuda.synchronize()
